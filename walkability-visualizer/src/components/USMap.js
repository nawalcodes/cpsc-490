        ...
        if (model === "EPA") {
            ...
            Promise.all([
                ...
                const ndaMap = new Map(
                    ndaData.map(d => [`${d.name},${d.state}`, +d.nda_index])
                );

                const enriched = cities
                    .filter(city => city.population > 50000)
                    .map(city => {
                        const id = `${city.name},${city.admin1_code}`;
                        const nda = ndaMap.get(id);
                        return {
                            ...city,
                            nda_index: nda ?? null
                        };
                    })
                    .filter(d => d.nda_index != null);
                ...
                const defs = svg.append("defs");
                const gradient = defs.append("linearGradient")
                    .attr("id", "legend-gradient-ndai")
                    .attr("x1", "0%").attr("x2", "100%")
                    .attr("y1", "0%").attr("y2", "0%");

                const stops = d3.range(10).map(i => ({
                    offset: `${(i / 9) * 100}%`,
                    color: d3.interpolatePuOr(1 - i / 9)
                }));
                ...
                legendGroup.append("rect")
                    .attr("width", legendWidth)
                    .attr("height", legendHeight)
                    .style("fill", "url(#legend-gradient-ndai)")
                    .style("stroke", "#999")
                    .style("stroke-width", 0.5);
